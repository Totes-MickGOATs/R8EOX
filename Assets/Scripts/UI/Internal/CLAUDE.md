...
- `OptionsOverlay.cs` — Self-contained options overlay (Canvas sortOrder=100, ScreenSpaceOverlay). Builds its entire UI hierarchy in code via Show(SettingsManager, Action onClosed). Contains an OptionsTabBar and six ScrollRect-backed tab panels (Video/Audio/Controls/Calibration/Gameplay/Profile); switches active panel on OnTabChanged. 0.25s CanvasGroup fade-in on open, 0.2s fade-out on close, both via unscaledDeltaTime. Escape key triggers Hide(). No prefab dependency.
