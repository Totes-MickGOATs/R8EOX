...
namespace R8EOX.Vehicle.Internal
{
    ...
    /// Force computation is delegated to <see cref="WheelForceSolver.Solve"/>.
    /// </summary>
    internal class RaycastWheel : MonoBehaviour
    {
        // ---- Constants ----
        ...
        private VehicleManager _cachedCar;
        private float _prevSpringLen, _rayLen;
        private bool _wasOnGround;
        private Vector3 _contactPoint;
        private Vector3 _contactNormal;
        private Vector3 _tireVelocity;
        private WheelForceResult _lastResult;

        // ---- Unity Lifecycle ----

        void Awake()
        {
            _rayLen = SuspensionMath.ComputeRayLength(_restDistance, _overExtend, _wheelRadius);
        ...
        /// <summary>Main physics entry point — called by VehicleManager each FixedUpdate.</summary>
        public void ApplyWheelPhysics(Rigidbody carRb, float dt)
        {
            if (_cachedCar == null) _cachedCar = carRb.GetComponent<VehicleManager>();
            ...
            _lastResult   = WheelForceSolver.Solve(in input);
            ...
            UpdateVisuals(dt);
            WheelRpm     = GripMath.ComputeWheelRpm(_lastResult.ForwardSpeed, _wheelRadius);
