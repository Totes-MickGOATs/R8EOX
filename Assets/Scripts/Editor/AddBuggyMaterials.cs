...
namespace R8EOX.Editor
{
    ...
    public static class AddBuggyMaterials
    {
        ...
        public static void Apply()
        {
            ...
            var blueSemi  = GetOrCreateMat("BlueSemi",
                new Color(0.18f, 0.45f, 0.90f, 0.85f), transparent: true);
            ...
            // Wheel visuals (grandchildren under WheelFL/FR/RL/RR)
        ...
        static void Assign(GameObject root, string childName, Material mat)
        {
            ...
            {
                Debug.LogWarning(
                    $"[AddBuggyMaterials] Child not found: {childName}");
                return;
            }
            ...
            {
                Debug.LogWarning(
                    $"[AddBuggyMaterials] No Renderer on: {childName}");
                return;
            }
        ...
        static void AssignChild(Transform parent, string childName, Material mat)
        {
            ...
            {
                Debug.LogWarning(
                    $"[AddBuggyMaterials] Child not found under {parent.name}: {childName}");
                return;
            }
            var r = child.GetComponent<Renderer>();
            if (r == null)
